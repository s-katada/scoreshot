...
| 小節の幅(#17) | 音符の間隔を 50〜200% で広げたり詰めたりする。楽譜ごとに保存。表示だけで、再生・書き出しには効かない |
| 改段(#18) | 選んだ小節から次の段にする。画像から読み取った楽譜は、紙と同じ所で最初から段を改める。印の無い所は幅に収まらなければ自動で改める |
...
pnpm check                         # 見た目では確かめにくい所の検証(155 項目)
