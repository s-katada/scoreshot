...
**紙のピアノ譜を写真から読み取り(OMR)、読み違いを直して、そのまま再生するアプリ**(macOS / iPadOS / iOS)。

**楽譜を一から書く使い方はしない。** 楽譜はいつも画像から読み取って作る(新規作成・MusicXML の読み込み・サンプルに戻すは #15 で削除した)。読み取りは原理的に 100% にならないので、読み違いを直すための編集機能は要る。
...
| タイ(#16) | 選んだ音を、次に鳴る同じ高さの音とつなぐ(小節線をまたいでもよい)。弧を描き、再生・MIDI では 1 つの長い音にする。読み取りではまだ拾わない |
| 臨時記号と調号(#10) | ♯ ♭ ♮ の付け外し。どこに表示するかは調号と小節の中の状態から決める |
| 基本情報(#11) | タイトル・調号・拍子・テンポの編集(読み取りで推し量り違えたときの直し口) |
| キーボード(#13) | A〜G で音、数字で音価、`.` で付点、`0` で休符、`T` でタイ、矢印・Delete・⌘Z / ⇧⌘Z・Space(再生)・Esc |
| 保存(#4)・楽譜一覧(#6) | AppData に自動保存。一覧から開く・複製・名前の変更・削除 |
| 書き出し(#5) | MusicXML と MIDI の書き出し |
...
#3 編集                     ✅
  ├ #11 基本情報・空の楽譜   ✅
  └ #10 臨時記号と調号       ✅
#5 MusicXML/MIDI 入出力     ✅(読み込みは #15 で削除)
#2 OMR                      ✅
#15 使わない機能の削除        ✅
#16 タイ                      ✅(読み取りでは拾わない)
#12 途中からの再生・一時停止 ✅
...
pnpm check                         # 見た目では確かめにくい所の検証(139 項目)
